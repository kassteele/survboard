...
server <- function(input, output, session) {
  ...
  #
  # Initital settings after disease selection ----
  #
  ...
  output$pl_out_time <- renderPlotly(
    expr = {
      ...
      if (all(dis$case.data$select.time)) {
        # If no time filter is active, plot the entire week sequence
        xlim <- week.seq %>% range %>% "+"(c(-4, 4))
      } else {
        # Else, plot the selected time range
  ...
  observeEvent(
    ...
    handlerExpr = {
      ...
      # Use plotly_brushed to return plot limits
      # (vs. plotly_brushing, which is instantanious,
      #  vs. plotly_selected, which returns selected bars)
      ...
      dis$case.data <- dis$case.data %>%
        mutate(select.time = WeekFS %>% between(
          left  = plot.info$x[1] %>% as.Date,
          right = plot.info$x[2] %>% as.Date))
    ...
    handlerExpr = {
      ...
      leafletProxy(
        ...
        addCircles(
          ...
          color = "#007bc7", opacity = 1, weight = 0.5,
          fillColor = "#007bc7", fillOpacity = 0.33,
